...
class VMS:
    ...
    def __init__(self, window, title):
        ...
        self._imgNovid = PIL.ImageTk.PhotoImage(PIL.Image.open(f"src/img/novid_{const.SIZE_VIEWCAM}.jpg"))
        ...
        self._canvas.pack(side = "right", fill = "y", padx = 10)
    ...
    def nodeNovid(self):
        ...
        for i in range(const.MAXCAMERAS):
            self._canvas.create_image((const.SIZE_VIEWCAM+5)*(i%4), (const.SIZE_VIEWCAM+5)*(i//4), image = self._imgNovid, anchor = tkinter.NW)
    ...
    def nodeInspector(self):
        ...
        self._inspector["columns"] = ("Descriptions", "Status")

        self._inspector.heading("#0", text = "Name", anchor = tkinter.NW)
        self._inspector.heading("Descriptions", text = "Descriptions", anchor = tkinter.NW)
        self._inspector.heading("Status", text = "Status", anchor = tkinter.NW)

        #Level - 1
        self._namepack = self._inspector.insert("", 1, text = "Pack", values = ("Test pack", "Connected"))
        self._inspector.insert(self._namepack, "end", text = "#1 Camera", values = ("Test camera", "Connected"))
        self._inspector.insert(self._namepack, "end", text = "#2 Camera", values = ("Test camera", "Connected"))
